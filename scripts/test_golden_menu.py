...
T1 = os.path.join(CACHE, '_g1.png')
T2 = os.path.join(CACHE, '_g2.png')
...
def detect_golden(img):
    ...
    for cnt in contours:
        ...
        cx, cy = x + w//2, y + h//2
        elements.append({'cx': cx, 'cy': cy, 'w': w, 'h': h, 'area': area, 'x': x, 'y': y})
    elements.sort(key=lambda e: e['area'], reverse=True)
    return elements

print("Golden Element Test", flush=True)
...
for i, g in enumerate(golden[:15]):
    ...
    # Return to menu (back a few times then reopen if needed)
    for _ in range(2):
        back()
        time.sleep(0.5)
    time.sleep(0.5)
    ...
    before = cap(T1)
    if before is None:
        print(f"  [{i}] SKIP: before None", flush=True)
        continue
    ...
    after = cap(T2)
    if after is None:
        print(f"  [{i}] SKIP: after None", flush=True)
        continue
    ...
    if d > 200000:
        cv2.imwrite(os.path.join(CACHE, f'golden_test_{i}_{x}_{y}.png'), after)
        results.append({"i": i, "x": x, "y": y, "w": g['w'], "h": g['h'], "area": float(g['area']), "diff": int(d), "tag": tag})

# Save
out = os.path.join(CACHE, 'golden_test_results.json')
