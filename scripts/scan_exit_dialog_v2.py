...
# 退出对话框通常在中央，取消/确认按钮在底部
# 缩小扫描范围：底部区域 y: 600-900, x: 400-1000
print("\n[扫描] 退出对话框按钮 (底部区域)")
print("="*70)
...
for x in range(400, 1000, 80):
    for y in range(600, 900, 60):
        ...
        # 对话框关闭会有较大变化 (>15%)
        if rate > 15:
            print(f"✅ ({x:4d}, {y:4d}): {changed:8d} px ({rate:5.1f}%)")
        elif rate > 5:
            print(f"⚠️ ({x:4d}, {y:4d}): {changed:8d} px ({rate:5.1f}%)")
