...
# scrcpy 截图（标准流强制使用，禁用回退）
from core.capability.input.screenshot.screen_capture import ScreenCapture, ScrcpyError
from core.capability.device.adb_manager import ADBDeviceManager


# ══════════════════════════════════════════════════════════════════
# scrcpy 全局截图单例（标准流专用，禁用回退）
# ══════════════════════════════════════════════════════════════════

_scrcpy_screen_capture: Optional[ScreenCapture] = None


def _get_scrcpy_screen_capture(device_addr: str = "localhost:16512") -> ScreenCapture:
    """获取/初始化 scrcpy 屏幕捕获器（单例）"""
    ...
    if _scrcpy_screen_capture is None:
        ...
        _scrcpy_screen_capture = ScreenCapture(adb_manager=adb_manager, config=config)
    return _scrcpy_screen_capture


def scrcpy_screencap(device_addr: str = "localhost:16512") -> Optional[bytes]:
    """通过 scrcpy 视频流截图（标准流专用，禁用回退）

    Args:
        device_addr: 设备地址（如 localhost:16512 或 emulator-5554）

    Returns:
        PNG 原始字节，失败抛出异常（不静默回退）
    """
    sc = _get_scrcpy_screen_capture(device_addr)
    png_data = sc.capture_screen(device_addr)
    if png_data is None:
        raise ScrcpyError(f"scrcpy 截图失败：设备 {device_addr}")
    return png_data


def scrcpy_screencap_unique(device_addr: str = "localhost:16512",
                            last_hash: Optional[str] = None) -> Tuple[Optional[bytes], Optional[str]]:
    """scrcpy 截图并去重（适配标准流去重逻辑）

    Returns:
        (image_bytes, md5_hash) 或 (None, same_hash) 如果画面未变化
    """
    img = scrcpy_screencap(device_addr=device_addr)
    if img is None:
        return (None, None)
...
def close_exit_dialog_with_verify(adb, screen_analyzer, tap_func):
    ...
    for i, (cx, cy) in enumerate(cancel_candidates):
        # 截图
        before = scrcpy_screencap(device_addr)
        if before is None:
            continue
        ...
        # 截图验证
        after = scrcpy_screencap(device_addr)
        if after is None:
            print("截图失败")
            continue
...
class FlowRecorder:
    ...
    def capture_screenshot(self, step_id: int, action: str) -> str:
        ...
        img = scrcpy_screencap(device_addr=self.device_addr) if self.device_addr else scrcpy_screencap()
        ts = time.time()
...
class StandardFlowExecutor:
    ...
    def _verify_screen_change(self):
        """验证画面变化 + 多源页面类型识别"""
        img = scrcpy_screencap(device_addr=self.device_addr)
        if img:
            import numpy as np
...
def main():
    ...
    def _close_exit_dialog():
        ...
        for cx, cy in cancel_candidates:
            ...
            # 验证是否关闭成功
            img_bytes = scrcpy_screencap(device_addr=device_addr)
