...
class ElementAnalyzer:
    ...
    def _capture_and_encode(self) -> Optional[str]:
        ...
        if isinstance(raw, bytes):
            if raw.startswith(b'/') or (b'\\' in raw and b':' in raw[:5]):
                # 文件路径（Unix / 或 Windows C:\）→ 读取并编码
        ...
        if isinstance(raw, str):
            if raw.startswith('/') or (':' in raw[:3] and '\\' in raw):
                # 文件路径（Unix / 或 Windows C:\）→ 读取并编码
                try:
    ...
    def verify_element(self, element: ElementKnowledge) -> bool:
        ...
        prompt = (
            ...
            f"返回JSON：{{\"exists\": true/false, \"confidence\": 0.0~1.0}}"
        )
        ...
        try:
            ...
            if result.get("status") == "success":
                ...
                if parsed:
                    return bool(parsed.get("exists", False))
                content = result.get("content", "")
                return "true" in content.lower() or "存在" in content
            return False
        except Exception:
            return False
