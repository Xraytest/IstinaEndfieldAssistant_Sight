...
- `adb_manager.py` — `ADBDeviceManager` (start-server, connect, shell, screencap); ADB at `3rd-part/adb/adb.exe`
  - **新增**：设备类型自动检测（MuMu、LDPlayer、BlueStacks、WSA、真机）
  - **新增**：智能推荐截图/触控配置（基于设备类型）
- `touch/` — `TouchManager` (增强版), `MaaFwTouchAdapter`
  - **新增**：支持多种触控方式（MaaTouch、minitouch、scrcpy、ADB）
  - **新增**：自动降级机制（首选方式失败后自动切换）
  - **新增**：Pipeline 任务执行（优先使用）
- `screenshot/` — `ScreenCapture` (增强版)
  - **新增**：7种截图方式（scrcpy、nemu_ipc、ldopengl、DroidCast、aScreenCap、ADB、uiautomator2）
  - **新增**：设备智能选择（auto 模式下自动使用推荐方案）
  - **新增**：自动降级链（scrcpy → MAA → ADB 等）
  - **新增**：卡死检测与连点保护
  - **新增**：性能监控与日志记录
...
## Gotchas
- **Windows-only**: ADB at `3rd-part/adb/adb.exe`, git at `3rd-part/git/bin/git.exe`
