...
src/gui/pyqt6/main.py
  ...
  -> MainWindow(agent_executor, gui_client, screen_capture, touch_executor, inference_manager, adb_manager, maaend_runtime)
    -> pages/
      ...
      - SettingsPage(config, agent_executor)
...
```
scripts/istina.py -> src/cli/istina.py
...
**当前状态**
- 统一入口 `istina.py` 保持，但 main() 只做解析+路由
...
class AndroidRuntime:
    """Android 交互独立实例 — 进程内单例，管理 ADB/触控全生命周期"""

    def __init__(self, adb_path="3rd-part/adb/adb.exe", device_address=None):
        self._client = AndroidRuntimeClient(serial=device_address or "default")
...
- Socket 路径动态生成（根据设备 serial），避免冲突；Windows 回退到 TCP localhost
...
- 通信路径：CLI 子进程 -> IstinaRuntime -> 跨进程 AndroidRuntimeClient -> 独立守护进程（_Daemon）
...
**命令路由**：GUI 只调用 `istina.py` 一个入口，通过子命令参数路由到不同功能。
...
### 调用方式
- **CLI**：`python scripts/istina.py llm prompt "分析当前页面"`
...
| `src/core/service/runtime.py` | `AndroidRuntime` 改为 `AndroidRuntimeClient` 跨进程调用，新增 `LlamaServerRuntime` 与 `LlmClient` 集成 |
...
1. **CLI 冒烟测试**: `python src/cli/istina.py device info` 验证共享层可用
