...
| **P1** | **`MaaEndRuntime.connect()` 未校验首次截图结果** | `maa_end/runtime.py:206-207` | 连接时调用了 `post_screencap()` 但未检查 `succeeded`。`_connected` 可能提前置为 `True`，而后续所有预览截图都失败。 |
...
| **P2** | **底层 MaaFW 环境不稳定** | 项目文档多处 | `MAAFW_BINARY_PATH` 双副本冲突（`3rd-part/maaend/agent/maafw/` vs 根目录 `maafw/`）、`input_methods=3` 硬编码、`_start_agent()` 静默失败，都会导致 `AdbController` 或截图后端行为异常，进一步放大预览失败率。 |
